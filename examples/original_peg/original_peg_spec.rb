...
input = <<END
  one = [a-z]
END
OriginalPeg.parse(input).to_ast.should == [:grammar,[:definition,"one",[:sequence,[:terminal_regexp,"[a-z]"]]]]
end
...
END
OriginalPeg.parse(input).to_ast.should == [:grammar,[:definition,"one",[:sequence, [:alternatives,[:terminal_string,"one"],[:terminal_string,'1']],[:terminal_string,"two"],[:alternatives,[:terminal_string,"three"],[:terminal_string,'3']]]]]
...
END
OriginalPeg.parse(input).to_ast.should == [:grammar,[:definition,"one",[:alternatives,[:sequence,[:terminal_string,"one"],[:terminal_string,"two"],[:terminal_string,"three"]],[:sequence,[:terminal_string,'1'],[:terminal_string,'2'],[:terminal_string,'3']]]]]
end
...
END
OriginalPeg.parse(input).to_ast.should == [:grammar,[:definition,"one",[:sequence,[:optional,[:terminal_string,"one"]],[:any_number_of,[:terminal_string,"two"]],[:one_or_more,[:terminal_string,"three"]],[:optional,[:alternatives,[:terminal_string,"four"],[:terminal_string,"five"]]]]]]
end
